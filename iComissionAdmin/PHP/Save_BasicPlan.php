...
if ($result->num_rows > 0) 
{
    while($row = $result->fetch_assoc())
    {
        ...
        {
            $sql1 = "insert into usermembershipplan(`PlanTypeID`, `CreatedBy`, `NoOfBids`, `NoOfSkills`,`NoOfEmpFollw`) values($PlanTypeID,$CreatorID,$NoOFBids,$NoOfSkills,$EmpFollw)";
            if ($conn->query($sql1)) 
            {
                        ...
                        $sql3 = "insert into membershipplanfeatures(`UserPlanID`, `UnlockRewards`, `UnlimitedProjectBookmarks`, `CustomCoverPhoto`)
                                values($value->UserPlanID,$UnlockRewards,$UnlimitedProjectBookmarks,$CustomCoverPhoto)";
                        if($conn->query($sql3))
                        {
                            $data = true;
                            $msg = 'data saved';
