        ...
        $value->AssignmentSeekerID;
                ...
                if($stmt2->execute())
                {
                                ...
                                if($stmt3->execute())
                                {
                                    ...
                                    {
                                       $count = 0;
                                        //echo (sizeof($JobTitle));
                                        for( $i=0;$i<=sizeof($JobTitle);$i++)
                                        {
                                                    $sql5 = "CALL usp_Insert_AssignmentSeekerExperienceDetails(?,?,?,?,?,?,?,?,?,?,?)";
                                            
                                                    $stmt5 = $conn->prepare($sql5);
                                            
                                                    $stmt5->bind_param("iiiisssssss",$value->AssignmentSeekerID,$TotalExp_Year[$i],$TotalExp_Month[$i],
                                                                            $Salary[$i],
                                                                                    $JobTitle[$i],$JobDescripion[$i],$ExpCompanyname[$i],
                                                                                        $ExpIndustry[$i],$Department[$i],$JobFromDate[$i],$JobToDate[$i]);
                                                    ...
                                                    {
                                                        $data = false;
                                                        $msg ='error in saving data  of Asssignment SeekerExperienceDetails';
